...
export default {
    ...
    setup(props) {
        ...
        const fetchProduct = async () => {
            if (isEditing.value) {
                ...
                if (response.success) {
                    ...
                    obj.value.federation_id = response.data.federation_id;
                }
            }
        };
        ...
        const fetchFederations = async () => {
            const response = await fetchAll('federations', {});
            if (response.success) {
        ...
        const handleFederationSelected = (itemSelected) => {
            obj.value.federation_id = (itemSelected.id)
        }

        return { obj, isEditing, saveData, error, validateForm, roles, isSelected, togglePermission, handleFederationSelected, federations };
    }
}
</script>
