...
export default {
    ...
    setup() {
        ...
        const columns = [
            ...
            { field: 'user', header: 'Autorizado por' },
            { field: 'authorized', header: 'Autorizado' },
...
<template>
    <CardComponent>
        <DataTable v-model:filters="filters" :value="data" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="id"
            ...
            :globalFilterFields="['athlete.name']">
            <template #header>
            ...
            <Column header="Atleta" headerStyle="width:20rem">
                <template #body="{ data }">
                    {{ data.athlete.name }}
                </template>
            </Column>
            <Column header="Autorizado por" headerStyle="width:20rem">
                <template #body="{ data }">
                    {{ data.user ? data.user.name : "Falta Autorización" }}
                </template>
            </Column>
            <Column header="Autorizado" headerStyle="width:20rem">
                <template #body="{ data }">
                    {{ data.authorized ? "Autorizado" : "No Autorizado" }}
                </template>
            </Column>
